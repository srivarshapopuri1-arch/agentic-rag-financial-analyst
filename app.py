...
st.caption(
    "Local, document-grounded research over public financial reports. "
...
with st.sidebar:
    ...
    if st.button("Index documents", use_container_width=True):
        ...
        except Exception as exc:  # noqa: BLE001 - surface indexing errors in the Streamlit UI
            st.error(f"Indexing failed: {exc}")

question = st.text_area(
    "Ask a financial question",
    placeholder=(
        "Example: What revenue was reported for the latest fiscal year? "
        "Cite the source."
    ),
)
...
if st.button(
    ...
    except Exception as exc:  # noqa: BLE001 - surface analysis errors in the Streamlit UI
        st.error(f"Analysis failed: {exc}")

st.divider()
st.caption(
    "Runs locally with Ollama. Use public or appropriately licensed documents only. "
    "Verify material conclusions in the original filing."
)